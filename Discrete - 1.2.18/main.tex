...
On Rearranging :
\begin{align}
    \mathbf{X[z]} \brak{1 - 2 z ^{-1}}= 0.5
\end{align}
    Applying Inverse Z-Transform on both sides:
\begin{align}
    \mathbf{X\brak{z}} - 2z^{-1}\mathbf{X\brak{z}} &= 0.5
\end{align}

Using the following Z-Transform properties:
\begin{align}
    \mathbf{X\brak{z}} &\longleftrightarrow x\brak{n} \\
    z^{-1}\mathbf{X\brak{z}} &\longleftrightarrow x\brak{n-1} \quad \text{(Time Delay Property)}\\
    0.5 &\longleftrightarrow 0.5\delta\brak{n} \quad \text{(Inverse Z-transform of constant)}
\end{align}

Applying Inverse Z-Transform directly:
\begin{align}
    \mathcal{Z}^{-1}\cbrak{X\brak{z} - 2z^{-1}X\brak{z}} &= \mathcal{Z}^{-1}\cbrak{0.5}
\end{align}
...
\begin{align}
    \boxed{x\brak{n} - 2x\brak{n-1} = 0.5\delta\brak{n}}
\end{align}

\begin{align}
\text{Given difference equation:} \quad 
x[n] - 2x[n-1] &= 0.5\,\delta[n]
\end{align}

\begin{align}
\text{Since ROC is } |z| < 2, \text{ the sequence is left-sided.} \\
\Rightarrow x[n] = 0 \quad \text{for } n \ge 0 \\
\Rightarrow x[0] = 0
\end{align}
...
\text{For } n \neq 0, \; \delta[n] = 0: \quad 
x[n] - 2x[n-1] &= 0 \\
\Rightarrow x[n] &= 2x[n-1] \\
\Rightarrow x[n-1] &= \frac{x[n]}{2}
\end{align}

\begin{align}
\text{Using backward recursion:} \\
x[-2] &= \frac{x[-1]}{2} = -0.125 \\
x[-3] &= \frac{x[-2]}{2} = -0.0625 \\
\vdots
\end{align}

\begin{align}
\text{Thus, the general form is:} \quad 
x[n] = -0.5 \cdot 2^n \quad \text{for } n \le -1
\end{align}
\end{frame}

\begin{frame}{Solution}
    If we plot the respective x[n] values , we will get the plot in slide 20 .
\end{frame}
...
\end{frame}
\begin{frame}[fragile]
    \frametitle{Python code (for plotting x[n] obtained from difference equation)}
    \begin{lstlisting}[language=Python]
import numpy as np
import matplotlib.pyplot as plt
...
plt.title(" x[n] vs n (through difference equation", fontsize=13, fontweight='bold')
plt.xlabel("n", fontsize=11)
plt.ylabel("x[n]", fontsize=11)
plt.grid(True, alpha=0.3)
plt.xlim(-10, 10)
plt.ylim(min(x) - 0.1, 0.1)
plt.tight_layout()
plt.show()
  \end{lstlisting}
\end{frame}
...
\begin{frame}{Plot}
    \begin{figure}
        \centering
        \includegraphics[width=1.01\linewidth]{plot3.png}
    \end{figure}
\end{frame}
\end{document}
